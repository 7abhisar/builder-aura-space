...
function calculateStats(posts: SentimentPost[]): SentimentStats {
  ...
  const avgSentiment =
    posts.length > 0
      ? (positivePosts * 1 + neutralPosts * 0 + negativePosts * -1) /
        posts.length
      : 0;
...
export const startMonitoring: RequestHandler = (req, res) => {
  const { hashtag, platforms = ["twitter"] } =
    req.body as StartMonitoringRequest;
...
export const getNewPosts: RequestHandler = (req, res) => {
  ...
  const newPosts = Array.from({ length: newPostsCount }, () =>
    generateMockPost(hashtag),
  );
